...
<br/>
  ...
  - Embedding Vector
    ...
    - 사전에서 보면 강사 라는 단어가 단어사전의 전체크기(5000)에서 100번째에 있으면 [0,0,0,...1 ... 0,0,0] -> 100번째에 1 존재 일때 이것은 Embedding Vector가 아님, 그냥 나열된 것일뿐 단순한 원핫백터임
    - 그래서 단어의 의미를 다차원 공간의 좌표(벡터)로 변환하는 기술
    - 예를 들어 단어들을 3차원 공간의 좌표로 만든다고 가정하면
      - 사과 -> [0.9, 0.1, 0.2] (과일 특성 높음, 기계 특성 낮음)
      - 바나나 -> [0.8, 0.2, 0.1]
      - 컴퓨터 -> [0.1, 0.9, 0.8]
      - 이 좌표값들의 배열이 Embedding Vector
