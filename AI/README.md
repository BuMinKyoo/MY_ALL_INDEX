  ...
  - 자연어 처리 (Natural Language Processing)
    - 개(개수) x 단(가장 긴 문장의 단어의 수) x 차(차원)
      ...
      - 차는, 단어 1개를 표현하는 float 데이터의 개수
        ...
        - 따라서,  실제로 맨처음 원핫백터일때 저는 = [1 0 0], 강사 = [0 1 0], 입니다 = [0 0 1] 이게 아니고, 저는 = [0 0 0 … 1 … 0 0 0] (1482 번째에 1 나머지는 0) 이런식으로 엄청 길다
          - 내가 써먹을 수 있는 한글 단어의 총 개수많큼 차원이 있는것
          - 그 차원의 수가 5000개라고 하면, 32x50x5000 이라는것이 기본으로 세팅이 되고, nn.Linear(5000, 512) 통과(행 뽑기)시키면 그게 워드 임베딩 벡터가 된다 (32x50x512)
