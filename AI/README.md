...
<br/>
  ...
  - 전체과정 설명
    ...
    - 28.결과값 10 x 512행렬을 5000개의 단어 사전과 매칭시기위해서 Linear 레이어(행렬 곱셈)진행(처음에 들어왔던 원핫배열크기), 이 5000개의 칸 각각에 점수가 들어있음
    - 29.Softmax통과, 5000개의 점수들이 확률(%)로 바뀌면서 가장 높은 확률을 가진 단어의 인덱스(예: 210번)가 최종 튀어나오게 되는 구조
...
<br/>
  ...
  - Multi-Head Attention 코드구현시 고려할점
    - Multi-Head Attention 는 10x512였을때 head가 8개여도, 10x64를 8번통과 시키는 for루프를 만드는 것이 아니라, 그것을 한번에 그냥 512를 통과시켜서 따로 사용하는 방식으로 만든다
      ...
      - 결국 10 x 512 행렬이 들어가서 가중치 행렬Q, K, V(512x512)행렬과 각각 곱을 하고 나온 Q와 K는 내적하고 그 후 V가 곱해지는것
