...
###### Top
  ...
  - [스마트포인터,Deref,강제역참조](#스마트포인터deref강제역참조)
...
# 스마트포인터,Deref,강제역참조
  - rust에서의 스마트 포인터
    - 러스트에서 '스마트 포인터'는 컴파일러 내부에 숨겨진 마법의 키워드나 특별한 원시 타입(Primitive Type)이 아닙니다. 그저 표준 라이브러리(std)에 일반적인 코드로 구현해 놓은 평범한 구조체(struct)들일 뿐
  - 스마트포인터 조건 2가지
    - Deref 트레이트
      - 구조체 앞에 *를 붙였을 때, 내부 데이터로 접근할 수 있게 해줍
      - 역참조 강제(Deref Coercion)
    - Drop 트레이트
      - 변수가 스코프(})를 벗어나 파괴될 때, 컴파일러가 자동으로 호출해 주는 정리(Clean-up) 함수
...
fn main(){
    let v = vec![1,2,3];
    print_array(&v);    // [1, 2, 3] Vec가 제거되서 간다
}
