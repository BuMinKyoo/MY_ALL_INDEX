...
<br/>
  ...
  - 가비지 컬렉션
    ...
    - 가비지 컬렉션의 원리
      - 스택을 전부 확인해서, 힙메모리에서 접근 하고 있지 않은 것들을 메모리를 해제해 버린다
    - 가비지 컬렉션의 문제점
      - 사용되지 않는 메모리를 즉시 정리하지 않음
      - GC가 메모리를 해제해야 하는지 판단하는 동안 애플리케이션이 멈추거나 버벅일 수 있음
    - 가비지 컬렉션 최적화 하기
      - 각각의 사용되는 빈도에 대해서 세대별로 구분하고, 그 세대별로 가비지 컬렉팅을 해서 매번 모든 메모리를 흩을 필요 없게 만든다
      - https://docs.microsoft.com/ko-kr/dotnet/standard/garbage-collection/fundamentals 

<br/>

  - 참조 카운팅
    - 가비지 컬렉션처럼, 개체에 대한 참조가 없을때 개체가 해제됨
