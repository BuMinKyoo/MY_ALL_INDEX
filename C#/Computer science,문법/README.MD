...
<br/>
  ...
  - 리스트(List)
    - List<int> <변수명> = new List<int>() -> 정의
    - List<int> <변수명> = new List<int>{ 1,2,3 } -> 정의
...
##### 37. LINQ : 데이터를 정렬하고, 추출하게 도와주는 C#문법중 하나, C#언어에 직접 쿼리 기능을 통합하는 방식을 기반으로 함
...
##### 38. 파일 입출력(여기에 있는 함수들은 파일을 알아서 열고 알아서 닫아주는 함수들임)
  - 파일 쓰기	
    - WriteAllText
    - WriteAllLines
    - etc..
